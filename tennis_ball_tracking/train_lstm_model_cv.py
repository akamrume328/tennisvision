...
class TennisLSTMTrainer:
    ...
    def prepare_sequences(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
        ...
        exclude_cols = ['label', 'video_name', 'frame_number', 'interpolated', 'interpolation_ratio', 'interpolation_confidence']
        ...
        for video in pd.unique(video_names):
            ...
            video_X, video_y = X_features[mask], y_labels[mask]
            video_conf = confidence_scores[mask] if confidence_scores is not None else None
